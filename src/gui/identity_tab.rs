...
pub fn draw_identity_manager(state: &mut GuiState, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    matrix_card_frame().show(ui, |ui| {
        ...
        if ui.button(RichText::new("⚡ Generate and Register").strong()).clicked() {
            if !state.alias_vault.is_empty() && !state.passphrase_vault.is_empty() {
                state.is_loading = true;
                state.status =
                    "Deriving post-quantum polynomial matrices in background...".to_string();
                    ...
                    for (index, id) in state.identities.iter().enumerate() {
                        ui.horizontal(|ui| {
                                ...
                                .clicked()
                            {
                                ...
                                state.selected_identity_pub_key = id.public_key_text.clone();
                            }

                            let btn_delete = egui::Button::new(
                                RichText::new("Delete").color(egui::Color32::from_rgb(255, 70, 70)),
                            );
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("📋 Copy to Clipboard").clicked() {
                            ctx.output_mut(|o| o.copied_text = state.selected_identity_pub_key.clone());
