...
pub fn draw_unlock(state: &mut GuiState, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            ui.label("Blind-decrypt Chameleon containers in constant time without metadata leakage.");
            ui.add_space(8.0);

    // SECTION 1: CONTAINER SELECTION
    matrix_card_frame().show(ui, |ui| {
        ui.label(RichText::new("1. CONTAINER INPUT").strong().color(MATRIX_BRIGHT_GREEN));
        ui.add_space(4.0);
    ...
    matrix_card_frame().show(ui, |ui| {
        ui.label(RichText::new("2. EXTRACTION PASSPHRASE").strong().color(MATRIX_BRIGHT_GREEN));
        ui.add_space(4.0);
        ...
        if ui.button(RichText::new("⚡ [ FILTER KEM ]").strong()).clicked() {
            if state.file_to_decrypt.is_empty() {
                state.status = "ERROR: Please select a .diegox container to decrypt.".to_string();
                return;
            }
            ...
            state.status =
                "Executing constant time (XOR) operations in background thread...".to_string();
                ...
                match QuantumCapsule::load_from_disk(&path) {
                    Ok(file_data) => {
                        ...
                        match worker_app.decrypt_blind(&file_data, &phrase) {
                            Ok((bytes, ext)) => {
                                let _ = tx.send(WorkerResult::DecryptionSuccess(bytes, ext));
                            }
                            Err(e) => {
    ...
    if let Some(bytes) = state.decrypted_bytes_output.as_ref() {
        ...
        matrix_card_frame().show(ui, |ui| {
            ...
            let target_filename = format!("{}_unsealed.{}", base_stem, state.decrypted_ext_output);
            ...
            ui.horizontal(|ui| {
                if ui.button(RichText::new("💾 [ QUICK EXPORT TO OUTPUT DIR ]").strong()).clicked() {
