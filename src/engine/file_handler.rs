...
impl QuantumCapsule {
    pub fn save_to_disk<P: AsRef<std::path::Path>>(&self, path: P) -> Result<(), String> {
        ...
        let mut file = File::create(path.as_ref())
            .map_err(|e| format!("Failed to create capsule file: {}", e))?;
        file.write_all(&encoded)
            .map_err(|e| format!("Failed to write capsule data: {}", e))?;
        Ok(())
    }

    pub fn load_from_disk<P: AsRef<std::path::Path>>(path: P) -> Result<Self, String> {
        let mut file =
            File::open(path.as_ref()).map_err(|e| format!("Capsule not found: {}", e))?;
        let mut buffer = Vec::new();
        file.read_to_end(&mut buffer)
            .map_err(|e| format!("Failed to read capsule: {}", e))?;
        bincode::deserialize(&buffer)
            .map_err(|e| format!("Corrupt capsule or invalid format: {}", e))
    }
}
